...
export default class CodeBeamerService {
	 ...
	 */
	public async getCodeBeamerCbqlResult(
		cbqlQuery,
		...
		try {
			...
			console.error(err);
			throw new Error(
				`Failed loading CBQL results. Status: ${err.status}`
			);
		}
	}
	 ...
	 */
	private async convertWiki2Html(
		wikiText: string,
		trackerItem
	): Promise<string> {
		wikiText = this.encodeSpecialChars(wikiText);

		const requestUrl = `${this.getApiBasePath()}/projects/${
			trackerItem.tracker.project.id
		}/wiki2html`;
		...
		try {
			...
			console.error(err);
			console.warn(
				`Failed converting ${trackerItem.name}'s description to HTML. It might therefore look a little weird. This is a known issue, which is being worked on.`
			);
			return wikiText;
		}
	}
	...
	public async getCodeBeamerUser(username?: string): Promise<any> {
		if (!username)
			username = this.store.getLocalSetting(LocalSetting.CB_USERNAME);
	 ...
	 */
	public async getCodeBeamerProjectTrackers(
		projectId?: string
	): Promise<any> {
		if (!projectId) {
			try {
		...
		try {
			...
			console.error(err);
			throw new Error(
				`Failed getting Trackers for project ${projectId}: ${err.status}`
			);
		}
	}
	...
	async getCodeBeamerTrackerDetails(trackerId: string): Promise<any> {
		...
		try {
			...
			console.error(err);
			throw new Error(
				`Failed getting details of Tracker ${trackerId}: ${err.status}`
			);
		}
	}
	...
	async getCodeBeamerOutgoingRelations(itemId: string): Promise<any> {
		...
		try {
			...
			return relations.outgoingAssociations.concat(
				relations.upstreamReferences
			);
		} catch (err) {
			console.error(err);
			throw new Error(
				`Failed getting outgoing relations for item ${itemId}: ${err.status}`
			);
		}
	}
	...
	async getCodeBeamerAssociationDetails(associationId: string) {
		...
		try {
			...
			console.error(err);
			throw new Error(
				`Failed getting association details for association ${associationId}: ${err.status}`
			);
		}
	}
	...
	async getTrackerSchema(trackerId: string): Promise<any> {
		...
		const response = await this.get(
			path,
			'',
			'Failed fetching Tracker schema'
		);
		return response.json();
	}
	...
	async enrichBaseCbItemWithDetails(cbItem): Promise<any> {
		...
		cbItem.tracker = await this.getCodeBeamerTrackerDetails(
			cbItem.tracker.id.toString()
		);

		cbItem.renderedDescription = '';
		if (cbItem.description)
			if (cbItem.descriptionFormat === 'Wiki') {
				cbItem.renderedDescription = await this.convertWiki2Html(
					cbItem.description,
					cbItem
				);
			} else {
	...
	async create(item: CreateCbItem): Promise<any> {
		let trackerId;
		try {
			trackerId = this.store.getBoardSetting(
				BoardSetting.INBOX_TRACKER_ID
			);
		} catch (error) {
			miro.showErrorNotification(
				'You must define an "Inbox Tracker ID" first!'
			);
		...
		try {
			...
			throw new Error(
				`Please verify the settings are correct. ErrorCode: ${err.status}`
			);
		}
	}
	 ...
	 */
	public static getQueryEntityNameForCriteria(
