...
async function init() {
  ...
  miro.board.ui.on("app_card:open", async (_event) => {
    ...
    try {
      itemId = (
        (await _event.appCard.getMetadata("item")) as Pick<CodeBeamerItem, "id">
      ).id;
    } catch (err: any) {
  ...
  console.info(
    `[codeBeamer-cards] Plugin v1.5.0 initialized. Experiencing issues? Let us know at https://github.com/codeBeamer-Extensions-and-Addons/codebeamer-miro/issues`
  );
}
